...
<form style="position: fixed;top: 35px;" id="formId" method="post" enctype="multipart/form-data">
    <input type="text"  name="id" required=""  hidden="hidden"
           style="display: none"  autocomplete="off" class="layui-input"  value="${task.id}">
    <div style="width: 70%;margin: auto;display: block">
        <div class="data_box">
            <span class="Data_name">开始时间</span>
            <input id="starttime1"   name="starttime" value="${task.starttime}" autocomplete="off" class="layui-input">
        </div>

        <div class="data_box">
            <span class="Data_name">结束时间</span>
            <input id="endtime1"  name="endtime"  value="${task.endtime}" autocomplete="off" class="layui-input">
        </div>
        <div class="data_box">
            <span class="Data_name">任务时长</span>
            <input id="workhours"   name="workhours" value="${task.workhours}"  placeholder="" autocomplete="off" class="layui-input">

        </div>
        <div class="data_box">
            <span class="Data_name">员工</span>
            <select id="first" name="empid" style="height:28px;width:80%;" onchange="setEmpId()">
                <option value="${task.empid}" >${task.empname}</option>
                <c:forEach items="${employeesList}" var="e">
                    <c:if test="${task.empname != e.name}">
        ...
        <div class="data_box">
            ...
            <input type="text" id="tasktitle" name="tasktitle" value="${task.tasktitle}" autocomplete="off" class="layui-input">
        </div>
        <div class="data_box">
            <span class="Data_name">任务详情</span>
            <textarea style="width: 75%;font-size: medium" id="taskdetails"  name="taskdetails" cols="30" rows="4" >${task.taskdetails}</textarea>
        </div>
