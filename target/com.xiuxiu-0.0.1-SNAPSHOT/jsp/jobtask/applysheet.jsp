...
<table style="overflow-x: auto;height:auto;margin-top:5px;" class="layui-table">
    ...
    <tr>
        ...
        <th nowrap  width="100px">员工</th>
        <th nowrap width="100px">总金额</th>
        <th nowrap width="100px">总申请时长</th>
        <th nowrap width="100px">待审核数</th>
        <th nowrap width="100px">待审核总时长</th>
        <th nowrap width="100px">拒绝申请数</th>
        <th nowrap width="100px">拒绝总时长</th>
        <th nowrap width="100px">加班总时长</th>
        <th nowrap width="100px">通过加班总时长</th>
        <th nowrap width="100px">请假总时长</th>
        <th nowrap width="100px">通过请假总时长</th>
        <th nowrap width="100px">业绩总时长</th>
        <th nowrap width="100px">通过业绩总时长</th>
        <th nowrap width="100px">审核人</th>
...
<script type="text/javascript">
    ...
    function searchPage(pageNum) {
        ...
        $.ajax({
            type : "POST",
            dataType : "json",
            url : "${pageContext.request.contextPath}/flow/queryApplication?pageNum="+pageNum+"",
            ...
            success : function(data) {
                ...
                $.each(data.list,function(index, list){
                    ...
                    str+='<td nowrap>'+list.totalduration+'</td>';
                    str+='<td nowrap>'+list.waitnum+'</td>';
                    str+='<td nowrap>'+list.waitduration+'</td>';
                    str+='<td nowrap>'+list.failnum+'</td>';
                    str+='<td nowrap>'+list.durationfail+'</td>';
                    str+='<td nowrap>'+list.totaljiaban+'</td>';
                    str+='<td nowrap>'+list.jiaban+'</td>';
                    str+='<td nowrap>'+list.totalqingjia+'</td>';
                    str+='<td nowrap>'+list.qingjia+'</td>';
                    str+='<td nowrap>'+list.totalyeji+'</td>';
                    str+='<td nowrap>'+list.yeji+'</td>';
                    if(list.confirmman == null){
                        str+='<td nowrap></td>';
                    }else {
                        str+='<td nowrap>'+list.confirmman+'</td>';
                    }
...
<script type="text/javascript">
    ...
    function lookdetails(idnum) {
        layer.open({
            type: 2,
            title: ['申请详情','color:black;background-color:#ffc000;'],
            shadeClose: true,
            shade: 0.8,
            area: ['80%', '85%'],
            content: '${pageContext.request.contextPath}/jsp/jobtask/applysheetdetails.jsp?idnum='+idnum,
          /*  end:function () {
                location.reload();
            }*/
        });
